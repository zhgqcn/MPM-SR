...
with torch.no_grad():
    ...
    torchvision.utils.save_image(HR_4_r, './test_outR_cpu_' + opt.outputHR2, padding=0)
    localtime = time.asctime( time.localtime(time.time()))
    print("R channel have tested and saved , time : ", localtime)
    
    HR_2_g, HR_4_g = testing(model_g, LR_g)
    torchvision.utils.save_image(HR_4_g, './test_outG_cpu_' + opt.outputHR2 , padding=0)
    localtime = time.asctime( time.localtime(time.time()))
    ...
    out_HR_2_b = Image.open('D:\\GraduationProjectBackUp\\TestImg\\all_black_1024.tif') 
    out_HR_4_b = Image.open('D:\\GraduationProjectBackUp\\TestImg\\all_black_2048.tif') 
