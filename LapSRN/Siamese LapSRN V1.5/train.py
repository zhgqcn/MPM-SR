...
from model import LasSRN_r, LasSRN_g
...
results_train = {'Avg. Loss': []}
results_val = {'Avg. Loss': [], 'Avg. PSNR1': [], 'Avg. PSNR2': []}
...
model_r = LasSRN_r().to(device)
model_g = LasSRN_g().to(device)
Loss = Loss()
...
def train(epoch):
        ...
        results_train['Avg. Loss'].append(float('%.4f'%(epoch_loss / len(training_data_loader))))

        if(epoch % 10 == 0):
            results_val['Avg. Loss'].append(float('%.4f' % (epoch_loss / len(training_data_loader))))
...
def val():
    ...
    with torch.no_grad():
        ...
        results_val['Avg. PSNR1'].append(float('%.2f'%(avg_psnr1 / len(val_data_loader))))
        results_val['Avg. PSNR2'].append(float('%.2f'%(avg_psnr2 / len(val_data_loader))))


def checkpoint(epoch):
    model_out_r_path = join(opt.save_models, 'LapSRN_r_epoch_{}.pth'.format(epoch))
    model_out_g_path = join(opt.save_models, 'LapSRN_g_epoch_{}.pth'.format(epoch))
    torch.save(model_r, model_out_r_path)
    torch.save(model_g, model_out_g_path)
...
for epoch in range(1, opt.nEpochs + 1):
    ...
    # optimizer_r = optim.SGD(model_r.parameters(), lr=lr, momentum=0.9, weight_decay=1e-5)
    optimizer_r = optim.Adagrad(model_r.parameters(), lr=lr, weight_decay=1e-5)
    optimizer_g = optim.Adagrad(model_g.parameters(), lr=lr, weight_decay=1e-5)
    train(epoch)
    ...
    if epoch % 10 == 0:
        ...
        data_frame_val = pd.DataFrame(
            data={'Avg. Loss': results_val['Avg. Loss'],
                  'Avg. PSNR1': results_val['Avg. PSNR1'],
                  'Avg. PSNR2': results_val['Avg. PSNR2']},
            index=range(1, k + 1)
        )
        data_frame_val.to_csv(opt.save_val_csv, index_label='Epoch')
        k = k + 1

    data_frame_train = pd.DataFrame(
        data={'Avg. Loss': results_train['Avg. Loss']},
        index=range(1, epoch + 1)
    )
    data_frame_train.to_csv(opt.save_train_csv, index_label='Epoch')
