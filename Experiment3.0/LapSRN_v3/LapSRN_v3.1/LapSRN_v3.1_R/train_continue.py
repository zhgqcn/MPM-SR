...
parser.add_argument('--pre_model', type=str, default='./r.pt', help='continue train model : load r model')
...
def checkpoint(epoch):
    ...
    state_r = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch':epoch, 'lr':lr}
...
if os.path.exists(opt.pre_model):
    ...
    for epoch in range(epoch_continue + 1, opt.nEpochs + 1):
        ...
        index=range(epoch_continue + 1, epoch + 1))
        data_frame.to_csv('./result-r-continue.csv', index_label='Epoch')
